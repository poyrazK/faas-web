...
**Done.** The console calls **202 of 255** unique paths in `api/openapi.yaml`.
Every one of the remaining 53 appears in the non-goals table at the bottom,
with the reason it is not a console surface.
...
- The coverage script reports 202/255, and every remaining path appears above.
- Every new path is mocked, and each error branch was exercised over HTTP.
- `npm run check` green: 418 tests across 64 files.
