...
const DiaTextReveal = forwardRef<HTMLSpanElement, DiaTextRevealProps>(
  (
    ...
    // LOCAL EDIT — server-render settled, not blank.
    //
    // Upstream starts the sweep at SWEEP_START, which builds a gradient whose
    // stops are all at negative positions and whose visible range is
    // `transparent 0% -> 100%`. With `color: transparent` that renders nothing,
    // so the prerendered HTML shipped the headline's accent invisible until the
    // bundle hydrated — the regression PR #38 fixed for this same element.
    //
    // Starting at SWEEP_END renders the settled text (buildGradient returns a
    // flat textColor once bandStart >= 100). `play()` sets SWEEP_START itself
    // before animating, so the sweep is unaffected.
    ...
    useEffect(() => {
      ...
      // LOCAL EDIT: only rewind when we are actually about to play. Rewinding
      // unconditionally blanked the text while it waited to come into view.
      if (isVisible) {
