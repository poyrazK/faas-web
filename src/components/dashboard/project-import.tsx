...
export function ProjectImport({
  ...
  const plan =
    ...
    scanResult.slug === slug &&
    scanResult.branch === branch
      ? scanResult.plan
      : null;
  useUnsavedGuard(Boolean(file) && !applied, undefined, '/dashboard/workflows/new');
  ...
  const runScan = () => {
    ...
    void scan
      ...
      .then((plan) => setScanResult({ plan, file, slug, branch }))
      .catch((err: unknown) =>
  ...
  return (
    <div className="flex flex-col gap-6">
      <p className="text-sm text-muted-foreground">
        Bring a repository from Kubernetes, Compose, Render, Fly, or serverless. Scan first, review
        the workloads, then apply the plan to create its apps.
      </p>
