...
let accountPlan: string | null = 'hobby';
vi.mock('./auth', () => ({
  useAuth: () => ({ account: accountPlan ? { plan: accountPlan } : null }),
}));
...
describe('DataProvider latest deployment metadata', () => {
  it.each(['free', 'hobby', null])(
    'uses one batch request and gates metrics for plan %s',
    async (plan) => {
      accountPlan = plan;
      ...
      expect(
        fetchMock.mock.calls.filter(([input]) => requestPath(input) === '/v1/apps/metrics')
      ).toHaveLength(plan === 'hobby' ? 1 : 0);
